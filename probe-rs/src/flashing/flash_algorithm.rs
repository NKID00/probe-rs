...
use probe_rs_target::{
    Core, CoreType, Endian, FlashProperties, MemoryRegion, PageInfo, RamRegion, RawFlashAlgorithm,
...
impl FlashAlgorithm {
    ...
    fn assemble_from_raw_with_data(
        ...
        let stack_align = Self::required_stack_alignment(core.core_type.architecture());
    ...
    pub(crate) fn assemble_from_raw_with_core(
        ...
        let core = &target.cores[core_index];
        ...
        Self::assemble_from_raw_with_data(algo, ram, data_ram, core)
    }
}
